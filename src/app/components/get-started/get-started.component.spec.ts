...
describe('GetStartedComponent', () => {
  ...
  it('should have a subtitle', () => {
    let html = fixture.nativeElement as HTMLElement;
    expect(html.querySelector('p')?.textContent).toEqual(
      'Choose up to 5 options of any combination for genre, artist, or song. First specify the category of your choice (artist, song, or genre), then type the name in.'
    );
  });

  it('should remove a pick from the chosen criteria when removePick() is called', () => {
    component.chosenCriteria = [
      { type: 'type', name: 'test1' },
      { type: 'type', name: 'test2', id: '2' },
    ];
    component.removePick(component.chosenCriteria[0]);

    expect(component.chosenCriteria).toEqual([
      { type: 'type', name: 'test2', id: '2' },
    ]);
  });
