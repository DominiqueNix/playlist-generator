  ...
  getOnePlayList(playlistId: any): Observable<{}> {
    return this.getAccessToken().pipe(
      switchMap((accessToken: any) => {
        ...
        return this.http.get(
          `https://api.spotify.com/v1/playlists/${playlistId}?market=ES`,
          { headers }
        );
      })
    );
  ...
  uriBuilder(obj: any): Observable<string> {
    if (obj.getStartedInput) {
      ...
      if (obj.getSpecificInput.popularity !== null) {
        specifics =
          specifics + `&target_popularity=${obj.getSpecificInput.popularity}`;
      }
      ...
      if (obj.getSpecificInput.acoustics !== null) {
        specifics =
          specifics + `&target_acousticness=${obj.getSpecificInput.acoustics}`;
      }
      if (obj.getSpecificInput.danceability !== null) {
        specifics =
          specifics +
          `&target_danceability=${obj.getSpecificInput.danceability}`;
      }
      if (artists !== '') {
